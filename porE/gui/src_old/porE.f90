...
contains
! 
subroutine porosity(struct,eval_method,probe_r,grid_a,grid_b,grid_c)
...
if (struct == 'ud') then                                            ! if user-defined structure is choosen
  ...
  open(unit=15,file='pypore.xyz',status='old',action='read')         ! read in the xyz file
  name_struct='pypore.xyz'
